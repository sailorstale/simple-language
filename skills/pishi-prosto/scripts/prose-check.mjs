...
function checkList(file, { n, text, raw }) {
  ...
  // Microsoft Style Guide, который цитирует 6а: пункт может занимать и одно
  // предложение, и пару коротких. Находка начинается там, где в пункт затолкан
  // целый разбор, потому что тогда список перестаёт читаться с одного взгляда.
  const sentences = body.split(/(?<=[.!?…])\s+/).filter((s) => s.trim().length > 3)
  if (sentences.length > 3)
    add(file, n, 'список', true, body, `${sentences.length} предложения в пункте — вынеси подробности вложенными подпунктами`)
}
...
// Свод 6д: документ распадается на фрагменты, потому что его сканируют глазами до
// чтения. Череда абзацев подряд без заголовка, списка, таблицы или цитаты — стена,
// которую читатель пропускает целиком.
function checkWallOfText(file) {
  ...
  const flush = (upto) => {
    if (run >= 6)
      add(file, start, 'стена', true, `${run} абзацев подряд без единого списка или подзаголовка`,
        'раздели кусок: вынеси перечисление в список, поставь подзаголовок или дай таблицу')
    run = 0
  }
  for (let i = 0; i < raws.length; i++) {
    const t = raws[i].trim()
    ...
    if (/^[#>|]/.test(t) || /^\s*(?:[-*]|\d+[.)])\s/.test(raws[i])) { flush(i); continue }
    if (run === 0) start = i + 1
    run++
  }
  flush(raws.length)
...
const NAMES = {
  ...
  'стена': 'Стена текста без форматирования',
